...
import { useAllHeaderBanners, useCreateHeaderBanner, useUpdateHeaderBanner, useDeleteHeaderBanner, type HeaderBanner } from '@/hooks/useHeaderBanners';
...
export default function AdminHeaderBanners() {
  const { data: banners, isLoading } = useAllHeaderBanners();
  const createBanner = useCreateHeaderBanner();
  const updateBanner = useUpdateHeaderBanner();
  const deleteBanner = useDeleteHeaderBanner();
  ...
  function openEdit(banner: HeaderBanner) {
    ...
    setFormInterval(banner.autoplay_interval ?? 5);
    setDialogOpen(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        active: formActive,
        autoplay_interval: formInterval,
      };
      if (editing) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">Banner do Header</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Banner slide abaixo do menu • Desktop: 1365×300px
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {banners.map(banner => (
            <div key={banner.id} className="bg-card rounded-xl border border-border overflow-hidden">
              <img src={banner.image_url} alt="" className="w-full h-36 object-cover" />
              <div className="p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground">⏱ {banner.autoplay_interval}s</span>
                </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input type="number" value={formInterval} onChange={e => setFormInterval(parseInt(e.target.value) || 5)} min={1} />
              </div>
